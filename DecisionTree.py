    ...
    for a in range(len(Data_Arr[0])):
        if a != LookingFor:
            ...
            for b in range(0,len(Data_Arr)-1):
                ...
                if Data_Arr[b][LookingFor] != Data_Arr[b+1][LookingFor]:

                    s = SSR(Data_Arr, a, (Data_Arr[b][LookingFor] + Data_Arr[b+1][LookingFor])/2 )
                    if s < S:
                        S = s
                        A = a
                        B = (Data_Arr[b][LookingFor] + Data_Arr[b+1][LookingFor])/2
